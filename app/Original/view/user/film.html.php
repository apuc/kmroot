...
<head>
    ...
    <title>Фильмы пользователя <?= $login ?></title>
        ...
        function deleteDir()
        {
            ...
            $(document).on('click', '.notifyjs-foo-base .yes', function() {
                ...
                $.ajax({
                    ...
                    "data": 'id=' + id,
                    "success": function (data) {
                        var num = parseInt($('.filmNumber').html()) - 1;
                        if (1 > num) {
                            num = '';
                        }
                        $('.filmNumber').html(num);
                            ...
                            for (var key in data) {
                                if (data.hasOwnProperty(key) && /^0$|^[1-9]\d*$/.test(key) && key <= 4294967294) {
                                    ...
                                    var html = '<div class="list-tile-item" data-id="' + item[5] + '">' +
                                        ...
                                        '<div class="list-tile-value list-tile-name"><a href="/film/' + item[0] + '/">' + item[4] + ' &nbsp;</a></div>' +
                                        '<div class="list-tile-value list-tile-name-eng"><a href="/film/' + item[0] + '/">' + item[3] + ' &nbsp;</a></div>';
                                    if (0 < item[6]) {
                                                ...
                                                success: function (data) {
                                                    if (0 == data) {
                                                        if (0 == $(folder).attr('data-id')) {
                                                            $(folder).notify("Фильм удалён", {
                                                                className: "success",
                                                                position: "left",
                                                                autoHideDelay: 2000
                                                            });
                                                        } else {
                                                            $(folder).notify("Фильм перенесён", {
                                                                className: "success",
                                                                position: "left",
                                                                autoHideDelay: 2000
                                                            });
                                                        ...
                                                        $("img.lazy").lazyload();
                                                        $(folder).notify("Фильм уже есть в данной коллекции", {
                                                            className: "info",
                                                            position: "left",
                                                            autoHideDelay: 2000
                                                        });
