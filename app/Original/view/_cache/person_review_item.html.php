...
<head>
    ...
    <title>Комментарии отзыва №<?= $reviewId ?> о персоне <?= $min[Person::TITLE] ?> | KINOMANIA.RU</title>
    <?php if ('' == $min[Person::NAME_RU]): ?>
        <meta name="description" content="Комментарии отзыва №<?= $reviewId ?> о персоне «<?= $min[Person::NAME_ORIGIN] ?>» на сайте KINOMANIA.RU. Обзоры новых сериалов, трейлеры, биографии актёров, обои на рабочий стол и многое другое из мира кино"/>
        <meta name="keywords" content="<?= $min[Person::NAME_ORIGIN] ?>, персона, отзыв №<?= $reviewId ?>, комментарии"/>
    <?php else: ?>
        <meta name="description" content="Комментарии отзыва №<?= $reviewId ?> о персоне «<?= $min[Person::NAME_RU] ?>» на сайте KINOMANIA.RU. Обзоры новых сериалов, трейлеры, биографии актёров, обои на рабочий стол и многое другое из мира кино"/>
        <meta name="keywords" content="<?= $min[Person::NAME_RU] ?>, <?= $min[Person::NAME_ORIGIN] ?>, персона, отзыв №<?= $reviewId ?>"/>
    <?php endif ?>
    ...
    <script>
        ...
        $(document).ready(function(){
                ...
                if (authProb) {
                    ...
                    if (1 < text.length) {
                        $.ajax({
                            ...
                            type: "POST",
                            data: "relatedId=<?= $reviewId ?>&parent=" + id + "&type=person&text=" + text,
                            success: function (data) {
                                if (1 == data) {
            ...
            if (authProb) {
                ...
                $.ajax({
                    url: '/user/' + login + '?handler=getPersonVote&relatedId=<?= $reviewId ?>',
                    type: "POST",
                    success: function (data) {
