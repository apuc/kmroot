...
class AJAX extends DefaultController
{
    ...
    public function search()
    {
        ...
        $result = $this->mysql()->query("SELECT t1.`id`, t1.`s`, t1.`image`, t1.`name_origin`, t1.`name_ru`, t1.`country`, t1.`year`, t1.`genre`,
                                            ...
                                            WHERE t1.`status` = 'show' AND t1.`premiere_usa` >= '{$year}-{$month}-01' AND t1.`premiere_usa` < '{$year}-{$month_b}-01' AND t1.`type` = '' ORDER BY t1.`premiere_usa` LIMIT 100
                                        ");
        while ($row = $result->fetch_assoc()) {
            ...
            $dEnd  = new \DateTime($row['premiere_usa']);
