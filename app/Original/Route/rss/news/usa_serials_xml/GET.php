<?php
namespace Original\Route_rss_news_usa_serials_xml;
...
class GET extends DefaultController
{
    ...
    public function index()
    {
        ...
        $chanel->addChild('title', 'Зарубежные сериалы');
        ...
        $image->addChild('title', 'Зарубежные сериалы');
        ...
        $result = $this->mysql()->query("SELECT t1.`id`, t1.`s`, t1.`image`, t1.`publish`, t1.`title`, t1.`anons` FROM `news` as `t1` 
                                        WHERE t1.`status` = 'show' AND `category` = 'Зарубежные сериалы' ORDER BY t1.`publish` DESC LIMIT 24");
