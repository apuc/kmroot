<?php
namespace Original\Route_rss_news_artkinomania_xml;
...
class GET extends DefaultController
{
    ...
    public function index()
    {
        ...
        $chanel->addChild('title', 'Арткиномания');
        ...
        $image->addChild('title', 'Арткиномания');
        ...
        $result = $this->mysql()->query("SELECT t1.`id`, t1.`s`, t1.`image`, t1.`publish`, t1.`title`, t1.`anons` FROM `news` as `t1` 
                                        WHERE t1.`status` = 'show' AND `category` = 'Арткиномания' ORDER BY t1.`publish` DESC LIMIT 24");
