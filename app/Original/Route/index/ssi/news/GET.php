<?php
namespace Original\Route_index_ssi_news;
...
class GET extends DefaultController
{
    ...
    public function index()
    {
        ...
        $result = $this->mysql()->query("SELECT t1.`id`, t1.`s`, t1.`image`, t1.`publish`, t1.`title`, t1.`anons`, t2.`comment` FROM `news` as `t1` 
                                        ...
                                        WHERE t1.`status` = 'show' AND t1.`center` = 'no' AND `category` = 'Новости кино' ORDER BY t1.`publish` DESC LIMIT 4");
        $active = true;
        ...
        $this->setTemplate('index/ssi/news.html.php');
    }
}