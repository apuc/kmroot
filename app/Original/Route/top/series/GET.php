<?php
namespace Original\Route_top_series;
    ...
    public function index()
    {
        ...
        if (!Wrap::$debugEnabled && $redisStatus && $redis->exists($key)) {
            ...
            $result = $this->mysql()->query(" SELECT t1.`id`, t1.`name_origin`, t1.`name_ru`, t2.`rate`, t2.`rate_count`
                                            ...
                                            WHERE t1.`status` = 'show' AND t2.`rate_count` > 10
                                            AND t1.`type` = 'series' OR t1.`type` = 'series_ru'
                                            ORDER BY t2.`rate` DESC LIMIT 100
                                            ");
        ...
        $this->setTemplate('top/series.html.php');
    }
}