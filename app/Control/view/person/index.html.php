...
<script type="text/javascript">
    $(document).ready(function(){
        $('#modalBtn').click(function(){
            $.ajax({
                url : '<?= $request->makeUrl('person/add') ?>',
                type: "GET",
        ...
        "aoColumns": [
            {
                "mRender": function (data, type, full) {
                    return '<a href="<?= Server::DEMO ?>/people/' + full[0] + '/" target="_blank" class="link"><em class="fa fa-external-link"></em></a> &nbsp; ' + full[0]
                }
            },
            {
                "orderable": false,
            ...
            },
            {
                "orderable": false,
                "mRender": function(data, type, full) {
                    full[2] = full[2].replace('<br />', ' ');
                    var content = '<a class="btn btn-info btn-xs" href="<?= $request->makeUrl('person/edit?id=') ?>' + full[0] + '/">Подробнее</a> &nbsp;' +
                        '<form method="post" style="display: inline-block;">' +
                        '<button type="submit" class="btn btn-danger btn-xs activeConfirm" data-title="Удалить персону `' + full[2] + '`?">Удалить</button>' +
