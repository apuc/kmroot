                    ...
                    <form method="get" id="searchForm">
                        <div class="form-group">
                            ...
                            <select name="type" id="type" class="form-control">
                                <option <?php if ('true' == $get->fetch('type', 'true')): ?> selected="selected" <?php endif ?> value="true">Новые</option>
                                <option <?php if ('false' == $get->fetch('type')): ?> selected="selected" <?php endif ?> value="false">Обработанные</option>
                            </select>
                        </div>
                    </form>
                </div>
...
<script type="text/javascript">
    ...
    var dTable = $('#dataTable').dataTable({
        ...
        stateSaveCallback: function (settings, data){
            localStorage.setItem('fixStorage', JSON.stringify(data));
        },
        stateLoadCallback: function (){
            try {
                return JSON.parse(localStorage.getItem('fixStorage'));
            } catch (e) {}
        },
        "paging":   true,
        "ordering": true,
        ...
        "aoColumns": [
            {
                "mRender": function (data, type, full) {
                    if ('film' == full[6]) {
                        return '<a href="/film/' + full[4] + '/" target="_blank" class="link"><em class="fa fa-external-link"></em></a> &nbsp; ' + full[0]
                    } else {
                        return '<a href="/people/' + full[4] + '/" target="_blank" class="link"><em class="fa fa-external-link"></em></a> &nbsp; ' + full[0]
                    }
                }
            },
            {
            ...
            },
            {
                ...
                "mRender": function(data, type, full) {
                    var content = '<form method="post" style="display: inline-block;">' +
                        '<button type="submit" class="btn btn-danger btn-xs activeConfirm" data-title="Удалить?">Удалить</button>' +
                        '<input type="hidden" name="id" value="' + full[0] + '" />' +
        ...
        "ajax": $.fn.dataTable.pipeline( {
            url: '?handler=getList&type=<?= $get->fetch('type', 'true') ?>',
