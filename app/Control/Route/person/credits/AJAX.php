...
class AJAX extends AdminController
{
    ...
    public function getFilm()
    {
        ...
        if (0 == intval($query)) {
            ...
            $result = $this->sphinx()->query("SELECT * FROM `film` WHERE MATCH('{$query}') LIMIT 6 OPTION ranker=sph04,field_weights=(name_ru=100,name_origin=90,search=80)");
            $idList = [];
            ...
            if (0 == count($idList)) {
                ...
                $result = $this->mysql()->query("SELECT `id`, `name_origin`, `name_ru` FROM `film` WHERE `id` IN ({$idList}) LIMIT 4");
                while ($row = $result->fetch_assoc()) {
            ...
            if (!empty($suggest) && false === stripos($query, $suggest)) {
                ...
                $result = $this->sphinx()->query("SELECT * FROM `film` WHERE MATCH('{$suggest}') LIMIT 6 OPTION ranker=sph04,field_weights=(name_ru=100,name_origin=90,search=80)");
                $idList = [];
                while ($rowData = $result->fetch_assoc()) {
                    $idList[] = $rowData['id'];
                }
