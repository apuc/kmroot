...
class Review extends DB
{
    ...
    public function delete($id = 0)
    {
        ...
        $this->db->query("DELETE FROM `film_review` WHERE `id` = {$id} LIMIT 1");
        $this->db->query("DELETE FROM `film_review_vote` WHERE `reviewId` = {$id}");
        $this->db->query("DELETE FROM `film_review_stat` WHERE `reviewId` = {$id}");

        $comment = new Comment($this->db);
        $result = $this->db->query("SELECT `id` FROM `comment` WHERE `relatedId` = {$id} AND `type` = 'film' AND `parent` = 0");
